...
module Apti
  ...
  class Apti
    ...
    def install(package)
      ...
      if display_packages(packages, operation, COLOR_INSTALL, question, aptitude_string.split(/\n/)[-2])
    ...
    def display_packages(packages, operation, color, question, download_size)
      ...
      if !dep_install.empty?
        puts "\n\033[1m#{I18n.t(:installing_for_dependencies)}\033[0m"
        dep_install.each { |package| display_package_line(package, max, COLOR_INSTALL) }
      end

      if !dep_remove.empty?
        puts "\n\033[1m#{I18n.t(:removing_unused_dependencies)}\033[0m"
